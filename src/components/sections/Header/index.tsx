    ...
    return (
        <div className="flex items-center relative">
            ...
            {primaryLinks.length > 0 && (
                <ul className="hidden mr-10 gap-x-10 lg:flex lg:items-center" data-sb-field-path=".primaryLinks">
                    <ListOfLinks links={primaryLinks} colors={colors} />
                </ul>
            )}
            {secondaryLinks.length > 0 && (
                <ul className="hidden ml-auto gap-x-2.5 lg:flex lg:items-center" data-sb-field-path=".secondaryLinks">
...
function HeaderLogoLeftPrimaryCentered(props) {
    ...
    return (
        <div className="flex items-center relative">
            ...
            {secondaryLinks.length > 0 && (
                <ul className="hidden lg:flex lg:items-center ml-auto gap-x-2.5" data-sb-field-path=".secondaryLinks">
                    <ListOfLinks links={secondaryLinks} />
                </ul>
            )}
            {(primaryLinks.length > 0 || secondaryLinks.length > 0) && <MobileMenu {...props} />}
...
function HeaderLogoLeftPrimaryRight(props) {
    ...
    return (
        <div className="flex items-center relative">
            ...
            {primaryLinks.length > 0 && (
                <ul className="hidden lg:flex lg:items-center ml-auto gap-x-10" data-sb-field-path=".primaryLinks">
                    <ListOfLinks links={primaryLinks} colors={colors} />
                </ul>
            )}
            {secondaryLinks.length > 0 && (
    ...
    return (
        <>
            <div className="flex items-center relative">
                ...
                {secondaryLinks.length > 0 && (
                    <ul className="hidden lg:flex lg:items-center gap-x-2.5 ml-auto" data-sb-field-path=".secondaryLinks">
                        <ListOfLinks links={secondaryLinks} />
                    </ul>
                )}
                {(primaryLinks.length > 0 || secondaryLinks.length > 0) && <MobileMenu {...props} />}
            ...
            {primaryLinks.length > 0 && (
                <ul className="hidden lg:flex lg:items-center lg:justify-center gap-x-10 mt-4" data-sb-field-path=".primaryLinks">
                    <ListOfLinks links={primaryLinks} colors={colors} />
                </ul>
            )}
        </>
        ...
        <>
            {links.map((link, index) => {
                if (link.__metadata.modelName === 'SubNav') {
                    return <LinkWithSubnav key={index} link={link} inMobileMenu={inMobileMenu} colors={colors} data-sb-field-path={`.${index}`} />;
                } else {
                    return (
                        <li
                            key={index}
...
function ListOfSubNavLinks({ links = [], inMobileMenu = false }) {
    return (
        <>
            {links.map((link, index) => (
                <li key={index}>
                    <Action {...link} className={classNames(inMobileMenu ? 'w-full justify-start' : 'text-sm')} data-sb-field-path={`.${index}`} />
                </li>
            ))}
        </>
    );
